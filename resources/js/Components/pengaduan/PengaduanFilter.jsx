      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500"
          />
        </div>

        <Listbox value={status} onChange={setStatus}>
          <div className="relative w-48">
            <Listbox.Button className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg flex justify-between items-center">
              {selectedStatus.label}
              <ChevronDown className="w-4 h-4" />
            </Listbox.Button>
            <Listbox.Options className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-md">
        ...
        <button
          onClick={() => onFilter({ search, status, kategori })}
